# BRIEF-UI-013: Föreningsadmin — redigera profil + ladda upp logo/cover

## Mål
Om user är admin för en klubb ska det finnas en "Redigera förening"-knapp på klubbsidan som öppnar en form där admin kan ändra namn, beskrivning, website, contact_email, samt ladda upp logo och cover-bild.

## Förutsättningar
Zivar skapar manuellt storage bucket `club-assets` i Supabase dashboard:
- Name: `club-assets`, Public: Yes, 10 MB, image/*

RLS-policies för bucket (SQL Editor):
```sql
...
Kontrollera också att `clubs`-tabellen har UPDATE-policy för admins:
```sql
...
## Berörda filer
- `apps/mobile/lib/data.ts` — updateClub + uploadClubAsset
- `apps/mobile/app/club/[id]/edit.tsx` — ny
- `apps/mobile/app/club/[id].tsx` — lägg till "Redigera förening"-knapp för admin
...
### 2. Skapa `apps/mobile/app/club/[id]/edit.tsx`:
```tsx
...
### 3. I `apps/mobile/app/club/[id].tsx`, bredvid "+ Skapa post"-knappen (om admin), lägg till "Redigera förening":
```tsx
{admin && (
...
- [ ] Endast admins ser redigera-knappen
- [ ] Logo + cover laddas upp till club-assets/{club_id}/...
- [ ] Text-ändringar sparas till clubs-tabellen

## Anti-patterns
- Glöm INTE update-policyn på `clubs` — annars kan RLS blockera update även för admins
- Cache-busta bild-URLs

## Commit
`BRIEF-UI-013: Club admin can edit profile and upload assets`

## Rollback
Git checkout + ta bort edit.tsx
