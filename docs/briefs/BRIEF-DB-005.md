# BRIEF-DB-005: User posts — databas-schema

## Mål
Skapa `user_posts`-tabellen så användare kan posta egna inlägg (bild + text), separat från club_posts.

## Berörda filer
- `supabase/migrations/010_user_posts_schema.sql` — ny

## Steg

Skapa `supabase/migrations/010_user_posts_schema.sql`:
...
## Verifiering
- [ ] Fil skapad
- [ ] Zivar behöver skapa storage bucket `user-posts` manuellt (samma policies som `avatars`)

## Commit
`BRIEF-DB-005: User posts schema`

## Rollback
```bash
git rm supabase/migrations/010_user_posts_schema.sql
```
