# BRIEF-UI-015: Stad-filter i Upptäck

## Mål
Lägg till en "Stad"-dropdown/chipsrad högst upp i Upptäck-fliken som filtrerar klubbar efter stad. Defaultar till "Alla städer".

## Blockerad av
BRIEF-DB-007 (cities-tabell + clubs.city_id).

## Berörda filer
- `apps/mobile/lib/data.ts` — getCities helper
- `apps/mobile/app/(tabs)/index.tsx` — lägg till city-filter
...
### 2. Uppdatera `apps/mobile/app/(tabs)/index.tsx` — lägg till city-filter och en ny chips-rad. **Full fil:**
```tsx
...
- [ ] Stad-chips visas högst upp, defaultar till "Alla städer"
- [ ] Val filtrerar listan

## Anti-patterns
- Använd INTE city_id-matchning ännu — befintliga clubs har city-text som fallback
- Överväg paginering om städer blir >20 (inte än)

## Commit
`BRIEF-UI-015: City filter chips in Discover`

## Rollback
Git checkout index.tsx + data.ts
